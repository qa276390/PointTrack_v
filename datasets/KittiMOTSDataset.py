...
class MOTSTrackCarsValOffset(Dataset):
    SEQ_IDS_TRAIN = ["%04d" % idx for idx in [0, 1, 3, 4, 5, 9, 11, 12, 15, 17, 19, 20]]
    SEQ_IDS_VAL = ["%04d" % idx for idx in [2, 6, 7, 8, 10, 13, 14, 16, 18]]
    #SEQ_IDS_VAL = ["%04d" % idx for idx in [16]] 
    ...
    def __init__(self, root_dir='./', mode="train", num_points=250, transform=None, random_select=False, az=False,
        ...
        self.transform = transform
        train = True if mode in 'training' else False
        val  = True if mode in "validation" else False
        test = True if mode in "testing" else False
...
class MOTSTrackCarsTrain(Dataset):
    ...
    def get_data_from_mots(self, index):
        ...
        #print('inst_names_inds', inst_names_inds)
        #print('inst_names', inst_names) # ['0009_1076', '0004_1074', ...]
        ...
        for pind, pi in enumerate(pickles):
            #print('pi', len(pi))
            #print('pi[0].keys()', pi[0].keys()) # ['frame', 'sp', 'img', 'mask', 'maskX']
            inst_id = pind + 1
            inst_length = len(pi)
            if inst_length > 2:
                ...
                start, end = 0, 1
                pis = pi[start:end + 1]
            for ii, inst in enumerate(pis): # pis = [pi[start], pi[mid], pi[end]]
                img = inst['img']
                ...
                sample['envs'].append(fg_num)
                sample['framestamp'].append(fstamp)
...
class MOTSTrackCarsTrainTransformer(Dataset):
    SEQ_IDS_TRAIN = ["%04d" % idx for idx in [0, 1, 3, 4, 5, 9, 11, 12, 15, 17, 19, 20]]
    SEQ_IDS_VAL = ["%04d" % idx for idx in [2, 6, 7, 8, 10, 13, 14, 16, 18]]
    TIMESTEPS_PER_SEQ = {"0000": 154, "0001": 447, "0002": 233, "0003": 144, "0004": 314, "0005": 297, "0006": 270,
                         "0007": 800, "0008": 390, "0009": 803, "0010": 294, "0011": 373, "0012": 78, "0013": 340,
